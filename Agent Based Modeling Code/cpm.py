...
# -*- coding: utf-8 -*-
"""
Created on Sun Jun 30 19:24:31 2024
...
class CPM:
    ...
    def make_init(self, init_type="circle", r=3, spacing=0.25):
        ...
        if init_type == "circle":
            ...
            dist_to_mid = (X0 - x_mid) ** 2 + (Y0 - y_mid) ** 2
            grid_choice = np.argsort(-dist_to_mid)
            
            x_max, x_min = max(X0), min(X0)
            y_max, y_min = max(Y0), min(Y0)
           
            k = 0
            cell_index = np.arange(self.n_cells)
            
            random.shuffle(cell_index[:self.n_cells * 4 // 5]) 

            while k < self.n_cells:
                i = k + len(grid_choice) - self.n_cells
                x0, y0 = X0[grid_choice[i]], Y0[grid_choice[i]]
    ...
    def generate_image_t(self, res=8, col_dict={"E": "red", "T": "blue"}, background=np.array([0, 0, 0, 0])):
        """
